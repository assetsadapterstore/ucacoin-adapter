...
import (
	...
	"errors"
	"fmt"
	"github.com/assetsadapterstore/ucacoin-adapter/ucacoinTransaction"
...
func (decoder *TransactionDecoder) SignBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			txHash := ucacoinTransaction.TxHash{
				Hash: keySignature.Message,
				Normal: &ucacoinTransaction.NormalTx{
					Address: keySignature.Address.Address,
					SigType: ucacoinTransaction.SigHashAll,
				},
			}
			...
			//签名交易
			/////////交易单哈希签名
			sigPub, err := ucacoinTransaction.SignRawTransactionHash(txHash.GetTxHashHex(), keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	var (
		txUnlocks  = make([]ucacoinTransaction.TxUnlock, 0)
		emptyTrans = rawTx.RawHex
		//sigPub     = make([]ucacoinTransaction.SignaturePubkey, 0)
		transHash     = make([]ucacoinTransaction.TxHash, 0)
		addressPrefix ucacoinTransaction.AddressPrefix
	)
	...
	for accountID, keySignatures := range rawTx.Signatures {
		...
		for _, keySignature := range keySignatures {
			...
			signaturePubkey := ucacoinTransaction.SignaturePubkey{
				Signature: signature,
				Pubkey:    pubkey,
			}
	...
	trx, err := ucacoinTransaction.DecodeRawTransaction(txBytes, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	for _, vin := range trx.Vins {
		...
		txUnlock := ucacoinTransaction.TxUnlock{
	...
	signedTrans, err := ucacoinTransaction.InsertSignatureIntoEmptyTransaction(emptyTrans, transHash, txUnlocks, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	pass := ucacoinTransaction.VerifyRawTransaction(signedTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if pass {
...
func (decoder *TransactionDecoder) createBTCRawTransaction(
	...
	var (
		err              error
		vins             = make([]ucacoinTransaction.Vin, 0)
		vouts            = make([]ucacoinTransaction.Vout, 0)
		txUnlocks        = make([]ucacoinTransaction.TxUnlock, 0)
		totalSend        = decimal.New(0, 0)
		destinations     = make([]string, 0)
		accountTotalSent = decimal.Zero
	...
	for _, utxo := range usedUTXO {
		in := ucacoinTransaction.Vin{utxo.TxID, uint32(utxo.Vout)}
		vins = append(vins, in)

		txUnlock := ucacoinTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: ucacoinTransaction.SigHashAll}
		txUnlocks = append(txUnlocks, txUnlock)
	...
	for to, amount := range to {
		...
		out := ucacoinTransaction.Vout{to, uint64(amount.IntPart())}
	...
	/////////构建空交易单
	emptyTrans, err := ucacoinTransaction.CreateEmptyRawTransaction(vins, vouts, lockTime, replaceable, addressPrefix)
	...
	////////构建用于签名的交易单哈希
	transHash, err := ucacoinTransaction.CreateRawTransactionHashForSig(emptyTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if err != nil {
...
func (decoder *TransactionDecoder) createOmniRawTransaction(
	...
	for _, utxo := range usedUTXO {
		...
		txUnlock := omniTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: ucacoinTransaction.SigHashAll}
		txUnlocks = append(txUnlocks, txUnlock)
