...
//UCABlockScanner bitcoin的区块链扫描器
type UCABlockScanner struct {
	*bitcoin.BTCBlockScanner
	ucaWM *WalletManager
}

//NewBTCBlockScanner 创建区块链扫描器
func NewUCABlockScanner(wm *WalletManager) *UCABlockScanner {
	bs := &UCABlockScanner{ucaWM: wm}
...
func (bs *UCABlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {

	return bs.ucaWM.getBalanceCalUnspent(address...)
